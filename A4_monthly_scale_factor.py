...
def spatial_figure(axs,data,lons,lats,colormap,colorbar_min,colorbar_max,tb_lef=True,tb_bot=True,bad_data=False): #c_bad,c_under,c_over,c_number=20,
	...
	cmap = discrete_cmap(14,colormap)   #  use 20 color bins, this can be changed
...
def daily_data(month,day):
	...
	return nh3_sat_B,lon_sat,lat_sat, nh3_emission_total_B,lon_emission,lat_emission
...
def monthly_mean_cal():	
	...
	for imonth in months:
		...
		for iday in days:	
			nh3_sat_B,lon_sat,lat_sat, nh3_emission_total_B,lon_emission,lat_emission = daily_data(imonth,iday)
...
title_list = 'Monthly scale factor (i$_{mon}$ emission/annual emission)'					###########
title_list1 = 'JAN'
title_list2 = 'FEB'
title_list3 = 'MAR'
title_list4 = 'APR'
...
colormap=cmap; colorbar_min=0;colorbar_max=0.14## change this accordingly
...
colormap=cmap;colorbar_min=0.02;colorbar_max=0.14 ## change this accordingly
...
#char = fig.colorbar(colormesh_1,orientation='horizontal',extend='both',cax=cbar_ax,ticks=np.round(np.arange(0,1.01,0.02)*(colorbar_max-colorbar_min)+colorbar_min,2))
char = fig.colorbar(colormesh_1,orientation='horizontal',extend='max',cax=cbar_ax,ticks=[0.00,0.02,0.04,0.06,0.08,0.10,0.12,0.14])
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'_A4_monthly_scale_factorkgkg-1_SO2_Scaled.png',bbox_inches='tight')   ##########	
fig.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'_A4_monthly_scale_factorkgkg-1_SO2_Scaled.ps', format='ps')
plt.show()
