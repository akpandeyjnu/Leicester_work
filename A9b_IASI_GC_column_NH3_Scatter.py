...
regridded_data_sat_uk_annual = np.nansum(regridded_data_sat_uk,axis=0)
...
iasi_nh3_uk_annual = np.nansum(iasi_nh3_uk,axis=0)
...
################# Scatter Plot ##################
GC_annual_1d= regridded_data_sat_uk_annual.flatten()
iasi_annual_1d= iasi_nh3_uk_annual.flatten() #.reshape(13482)

x11 = GC_annual_1d/1e15
y11 = iasi_annual_1d/1e15
...
ax = plt.subplot(111);
plt.title(title_list1, fontsize = 41, y=1.01)

plt.plot(x1,y1, 'o', color='black',markersize=11)
...
plt.xlabel("GC column (10$^{15}$ molecules/cm$^2$)", fontsize = 36)
plt.ylabel("IASI Column (10$^{15}$ molecules/cm$^2$)", fontsize = 36)
plt.gca().set_aspect('equal', adjustable='box')
plt.draw()
ax.yaxis.get_offset_text().set_size(35)
plt.yticks(fontsize = 35)
ax.xaxis.get_offset_text().set_size(35)
plt.xticks(fontsize = 35)
plt.legend(('Data','RegressionLine','1:1 Line',),fontsize = 35)
add2plt=("y = {a:6.2f}x + {b:6.2f}".\
		format(a=regres_annual[0],b=regres_annual[1]))
plt.text(35,75,add2plt, fontsize=32,\
		ha='left', va='center')#, transform=ax.transAxes)
#add2plt=("R$^2$ = {a:6.2f}".format(a=correlate_annual[0]*correlate_annual[0]))
#plt.text(60,70, add2plt, fontsize=16,\
#		ha='left', va='center')#, transform=ax.transAxes)
add2plt=("r = {a:6.2f}".format(a=r_value1))
plt.text(35,65, add2plt, fontsize=32,\
		ha='left', va='center')#, transform=ax.transAxes)
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'_A9b_IASI_GC_Column_NH3_scatter.png',bbox_inches='tight')
fig.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'_A9b_IASI_GC_Column_NH3_scatter.ps', format='ps')
plt.show()
